package com.kinder.kinder_ielts.response_message;

public class TutorMessage {

    public static final String FOUND_SUCCESSFULLY = "Lấy thông tin giảng viên dùng thành công.";
    public static final String MULTIPLE_FOUND_SUCCESSFULLY = "Tìm kiếm nhiều giảng viên thành công.";
    public static final String CREATED = "Tạo giảng viên thành công";

    public static final String NOT_FOUND = "Không tìm thấy giảng viên";
    public static final String CREATE_FAILED = "Tạo giảng viên thất bại";
    public static final String PARTIALLY_NOT_FOUND = "Một số giảng viên không tìm thấy hoặc đã bị xóa";
}
